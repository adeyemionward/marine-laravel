...
class NewsLetterController extends Controller
{
    ...
    public function getStats()
    {
        ...
        $totalUsers = User::whereNotNull('email_verified_at')->whereNotNull('email')->count();
    ...
    public function getRecipients(Request $request)
    {
            ...
            if ($request->status === 'subscribed') {
                $query->where(function($q) {
                    $q->whereNull('newsletter_subscribers.status')
                      ->orWhere('newsletter_subscribers.status', '!=', 'unsubscribed');
                });
            } else if ($request->status === 'unsubscribed') {
                $query->where('newsletter_subscribers.status', 'unsubscribed');
            }
        }
        ...
        $recipients->getCollection()->transform(function($user) {
            $user->is_subscribed = is_null($user->subscription_status) || $user->subscription_status !== 'unsubscribed';
            return $user;
        });
